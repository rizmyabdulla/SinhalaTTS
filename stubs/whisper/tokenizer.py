"""Whisper ``Tokenizer`` stub for CosyVoice3 on Python 3.12.

CosyVoice imports ``Tokenizer`` from ``whisper.tokenizer`` when loading the
training yaml. Only the dataclass is needed here; CosyVoice builds tiktoken
encodings in ``cosyvoice.tokenizer.tokenizer``.
"""
...
@dataclass
class Tokenizer:
    """Thin wrapper around ``tiktoken`` (mirrors openai-whisper v20231117)."""
    ...
    def all_language_codes(self) -> Tuple[str]:
        return tuple(self.decode([lang_id]).strip("<|>") for lang_id in self.all_language_tokens)
